...
#give a file that can be imported into R and then use gplots venn diagram - see vennDiagram.R

#Perform GREAT GO on the bed file generated in the plotvenn R script - remember to remove Prn/PrnP from the GO analysis


#After generating the intersect file, subset the sites that are constantly up/downregulated in AD samples and annotate gene names
https://support.bioconductor.org/p/67118/



#######################################################################################
#19082018-21082018 PUTATIVE ENHANCER TRACK
#Generating pseudoreplicate super enhancer track from K27ac peak calling files previously generated above - see following code for peak calling settings
cat K27ac_samples.txt | TMPDIR=/NGS_Data/tmp parallel -j 35 "(macs2 callpeak -g 1.87e9 -q 0.05 -t {} -c /NGS_Data/Andrew/Bowtie2BAM_sort/TI_bam_sort/*.sorted.bam -n {} --outdir /NGS_Data/Andrew/Pseudoreplicate_BAM/H3K27ac_pseudo_peaks/ ) \
2>/NGS_Data/Andrew/Pseudoreplicate_BAM/H3K27ac_pseudo_peaks/{}.log"
#sort then merge K27ac peak files for a merged track
cat K27ac_sample_summits.txt | TMPDIR=/NGS_Data/tmp parallel -j 35 "(bedtools sort -i {} > {}_sorted_summits.bed)" 2> {}_stderr.txt
cat K27ac_TG3_pseudo2.sorted.bam_summits.bed_sorted_summits.bed \
K27ac_TG3_pseudo1.sorted.bam_summits.bed_sorted_summits.bed \
K27ac_TG6_pseudo1.sorted.bam_summits.bed_sorted_summits.bed \
K27ac_TG6_pseudo2.sorted.bam_summits.bed_sorted_summits.bed \
K27ac_TG12_pseudo1.sorted.bam_summits.bed_sorted_summits.bed \
K27ac_TG12_pseudo2.sorted.bam_summits.bed_sorted_summits.bed \
K27ac_WT3_pseudo1.sorted.bam_summits.bed_sorted_summits.bed \
K27ac_WT3_pseudo2.sorted.bam_summits.bed_sorted_summits.bed \
K27ac_WT6_pseudo1.sorted.bam_summits.bed_sorted_summits.bed \
K27ac_WT6_pseudo2.sorted.bam_summits.bed_sorted_summits.bed \
K27ac_WT12_pseudo1.sorted.bam_summits.bed_sorted_summits.bed \
K27ac_WT12_pseudo2.sorted.bam_summits.bed_sorted_summits.bed \
K27ac_WT6_pseudo2.sorted.bam_summits.bed_sorted_summits.bed \
K27ac_WT24_pseudo2.sorted.bam_summits.bed_sorted_summits.bed \
> K27ac_allmerge_peaks_summits.bed
bedtools sort K27ac_allmerge_peaks_summits.sorted.bed
#merge neighboring peak locations and then this file is ready to remove 5kb regions from
bedtools merge -i K27ac_allmerge_peaks_summits.sorted.bed > K27ac_allmerge_peaks_summits.sorted.merged.bed
#this file is now all sites of peaks found in WT/TG samples ready to make enhancer track

#extend the mm10 knowngene bed file for (\tsv) $2 -5000, $3 + 5000, $7 -5000, $8 + 5000
#download mm10knowngene from UCSC table browser
#use Awk to find a location where 
#https://www.biostars.org/p/250091/
awk '($6 == "+") { print $0 }' mm10_knowngene_edited.bed | awk 'BEGIN{ OFS="\t" }($2 > 5000){ print $1, ($2 - 5000), ($2 + 5000), $4, $5, $6, $7, $8, $9, $10, $11, $12}' > UCSCKnowngene.tss.for.padded.bed
awk '($6 == "-") { print $0 }' mm10_knowngene_edited.bed | awk 'BEGIN{ OFS="\t" }($3 > 5000){ print $1, ($3 - 5000), ($3 + 5000), $4, $5, $6,  $7, $8, $9, $10, $11, $12}' > UCSCKnowngene.tss.rev.padded.bed
#Now we can merge the padded file together into a single file 
cat UCSCKnowngene.tss.for.padded.bed  UCSCKnowngene.tss.rev.padded.bed > UCSCKnowngene_5kb_padded12.bed
bedtools sort -i UCSCKnowngene_5kb_padded12.bed > UCSCKnowngene_5kb_padded.sorted12.bed
bedtools merge -i UCSCKnowngene_5kb_padded.sorted12.bed > UCSCKnowngene_5kb_final_merged_sorted.bed

#Now we subtract the UCSCKnowngene_5kb_final_merged_sorted.bed from our peak track K27ac_allmerge_peaks_summits.sorted.merged.bed
#A is going to be the K27ac sample, B is going to be the knowngene track
bedtools subtract -a K27ac_allmerge_peaks_summits.sorted.merged.bed -b UCSCKnowngene_5kb_final_merged_sorted.bed > K27ac_knowngene_NeuN_putative_enhancer_track.bed 
#######################################################################################
#Enhancer heatmap generation
#DeepTools Heatmap generation
computeMatrix reference-point -S /NGS_Data/Andrew/Pseudoreplicate_BAM/bigwig/H3K27ac_bw/*WT3*.bw /NGS_Data/Andrew/Pseudoreplicate_BAM/bigwig/H3K27ac_bw/*TG3*.bw -R /NGS_Data/Andrew/Pseudoreplicate_BAM/Pseudoreplicate_K27ac_putative_enhancer/K27ac_knowngene_NeuN_putative_enhancer_track.bed -o ./K27ac_AD3_Enhancer_pseudo_matrix -a 5000 -b 5000 -p max -bl /NGS_Data/Andrew/bigwig/mm10.blacklist.bed
computeMatrix reference-point -S /NGS_Data/Andrew/Pseudoreplicate_BAM/bigwig/H3K27ac_bw/*WT12*.bw /NGS_Data/Andrew/Pseudoreplicate_BAM/bigwig/H3K27ac_bw/*WT24*.bw -R /NGS_Data/Andrew/Pseudoreplicate_BAM/Pseudoreplicate_K27ac_putative_enhancer/K27ac_knowngene_NeuN_putative_enhancer_track.bed -o ./K27ac_WT12v24_Enhancer_pseudo_matrix -a 5000 -b 5000 -p max -bl /NGS_Data/Andrew/bigwig/mm10.blacklist.bed

#have to make sure the bed file doesn't have any headers etc otherwise computeMatrix will not work
for file in *matrix ; do 
plotHeatmap --matrixFile ${file} --zMin 0 --zMax 7.5  --outFileName ${file}_heatmap.eps --dpi 600 --plotFileFormat eps -p max
done
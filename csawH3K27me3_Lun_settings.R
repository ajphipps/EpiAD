---
title: "H3K27me3_12m_Lun_AD_pseudo_binned_04072018"
...
```
The settings for this analysis have been pulled from: https://www.sciencedirect.com/science/article/pii/S2213596015001506
...
```{r}
#import required bam files - for now stick to single timepoint WT vs TG for H3K27me3 - but not sure if it would be better to load all data
...
H3K27me3_bam.files <- file.path("/NGS_Data/Andrew/Pseudoreplicate_BAM/H3K27me3_pseudoreplicate_bam", c("H3K27me3_WT12_pseudo1.sorted.bam", "H3K27me3_WT12_pseudo2.sorted.bam", "H3K27me3_TG12_pseudo1.sorted.bam", "H3K27me3_TG12_pseudo2.sorted.bam"))
```
We will now make a design matrix for these samples (keep the same order as samples)
```{r}
#design matrix
design <- model.matrix(~factor(c('WT12', 'WT12', 'TG12', 'TG12')))
...
window.width <- 2000 #will partition into windows of 150bp - for TF set small window, but for histone marks set window relevent \
...
data <- windowCounts(H3K27me3_bam.files, ext=frag.length, width =window.width, param = parameters)
```
...
plot1 <- correlateReads(H3K27me3_bam.files, max.delay, param = dedup.on)
...
bin.size <- 2000L 
binned1 <- windowCounts(H3K27me3_bam.files, bin=TRUE, width=bin.size, param=parameters)
...
filtered.keep <- filtered.stat$filter >log2(0.5)
```
...
abline(v=global.bg[1]+log2(0.5), col="blue", lwd=2)
...
binned <- windowCounts(H3K27me3_bam.files, bin=TRUE, width=10000, param=parameters)
...
#demo <- windowCounts(H3K27me3_bam.files, bin=TRUE, width=5000, param=parameters)
...
#demo <- windowCounts(H3K27me3_bam.files, bin=TRUE, width=15000, param=parameters)
#normOffsets(demo, se.out=FALSE)
...
for (i in seq_len(length(H3K27me3_bam.files)-1)) {
  cur.x <- adj.counts[,1]
...
#need: filtered.data and filtered.data, original data: H3K27me3_bam.files, and design matrix
#setting up the data
...
mergedwindowsH3K27me3 <- mergeWindows(rowRanges(filtered.data), tol = 1000, max.width = 8000L)
mergedwindowsH3K27me3$region
#assigning combined P value for merged windows
p.mergedwindowsH3K27me3 <- combineTests(mergedwindowsH3K27me3$id, results$table)
...
summary(width(mergedwindowsH3K27me3$region))
```
...
```{r}
merged <- mergeWindows(rowRanges(filtered.data), tol = 6000L, max.width = 12000L)
merged$region
...
```{R}
all.results <- (data.frame(as.data.frame(merged$region),tabcom, anno.merged))
...
```{R}
#library(Gviz)
#cur.region <- GRanges("chr2", IRanges(25315551, 25319300))
#collected <- list()
...
export.bed(test, "H3K27me3_AD12-3FC_DBsites_03072018.bed")
head(read.table("H3K27me3_AD12-3FC_DBsites_03072018.bed"))
```
...
```{r}
library(rtracklayer)
save.image(file = "H3K27me3_12m_AD-3FC_csaw_pseudo_environment_03072018.Rda")
write.csv(all.results, file="H3K27me3_12m_AD-3FC_bcluster_DB_pseudo_all_results_03072018.csv")
```
